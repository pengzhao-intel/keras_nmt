...
class GRU(gof.Op):
    ...
    def make_node(self, x, W_x, W_h, h_init_state, b):
        """
        ...
        h_init_state = tensor.as_tensor_variable(h_init_state)
        ...
        assert h_init_state.ndim is 2

        out = [x.type(), x.type(), x.type(), x.type(), x.type()]


        # output workspace (hid, zt, rt, hcan, hht)
        #out = out + [inp[0].type(), inp[0].type(), inp[0].type(), inp[0].type()]

        return gof.Apply(self, [x, W_x, W_h, h_init_state, b], out)
    ...
    def c_code(self, node, name, inputs, outputs, sub):
        ...
        ccode = """
            ...
            printf("Wx size %%d, %%d\\n",PyArray_DIMS(%(W_x)s)[0],PyArray_DIMS(%(W_x)s)[1]);
            //// step 1. Dot(X, (W_x))
    ...
    def grad(self, inp, grads):
        X, Wx, Wh, hid_init,b = inp
        gz = grads[0]

        hid, zt, rt, hcan, hht = GRU(hid=self.hid,
                                     step=self.step,
                                     dim=self.dim,
                                     return_sequences=self.return_sequences)(X, Wx, Wh, hid_init,b )
        """
